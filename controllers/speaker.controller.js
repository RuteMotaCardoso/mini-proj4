...
function readID(req, res) {
    ...
    const post = { idSpeaker: idspeaker };
    connect.con.query('SELECT idSpeaker, nome, foto, bio, link, filiacao, active, filiacao, linkedin, twitter, facebook, cargo FROM speaker where ? order by idSpeaker desc', post, function(err, rows, fields) {
        if (err) {
            console.log(err);
            res.status(jsonMessages.db.dbError.status).send(jsonMessages.db.dbError);
        }
