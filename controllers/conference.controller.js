...
// MembroComissao
function readConfMembroComissao(req, res) {
    //criar e executar a query de leitura na BD
    const idConf = req.sanitize('idconf').escape();
    const post = { idConference: idConf };
    const query = connect.con.query(
        'SELECT idMembro, nome, instituto, pais, active '
        + ' FROM membroComissao mc  '
        + ' WHERE mc.IdMembro in (select idMembro from conf_MembroComissao where ?) ' 
...
function saveConfMembroComissao(req, res) {
    //receber os dados do formuário que são enviados por post
    const idConf = req.sanitize('idconf').escape();
    const idMembro = req.sanitize('idMembro').escape();
    if (idMembro != "NULL" && idConf != "NULL" && typeof(idMembro) != 'undefined' && typeof(idConf) != 'undefined') {
        const post = { idMembro: idMembro, idConference: idConf };
        //criar e executar a query de gravação na BD para inserir os dados presentes no post
        const query = connect.con.query('INSERT INTO conf_membroComissao SET ?', post, function(err, rows, fields) {
            console.log(query.sql);
            if (!err) {
                res.status(jsonMessages.db.successInsert.status).send(jsonMessages.db.successInsert);
            }
...
function deleteConfMembroComissao(req, res) {
    //criar e executar a query de leitura na BD
    const idConf = req.sanitize('idconf').escape();
    const idMembro = req.sanitize('idMembro').escape();
    const params = [idConf, idMembro];
    console.log(params);
    const query = connect.con.query('DELETE FROM conf_membroComissao where idConference = ? and idMembro = ?', params, function(err, rows, fields) {
        console.log(query.sql);
        if (!err) {
            res.status(jsonMessages.db.successDelete.status).send(jsonMessages.db.successDelete);
        }
...
// Voluntario
function readConfVoluntario(req, res) {
    //criar e executar a query de leitura na BD
    const idConf = req.sanitize('idconf').escape();
    const post = { idConference: idConf };
    const query = connect.con.query(
...
function saveConfVoluntario(req, res) {
    //receber os dados do formuário que são enviados por post
    const idConf = req.sanitize('idconf').escape();
    const idVoluntario = req.sanitize('idVoluntario').escape();
    if (idVoluntario != "NULL" && idConf != "NULL" && typeof(idVoluntario) != 'undefined' && typeof(idConf) != 'undefined') {
        const post = { idVoluntario: idVoluntario, idConference: idConf };
        //criar e executar a query de gravação na BD para inserir os dados presentes no post
...
function deleteConfVoluntario(req, res) {
    //criar e executar a query de leitura na BD
    const idConf = req.sanitize('idconf').escape();
    const idVoluntario = req.sanitize('idVoluntario').escape();
    const params = [idConf, idVoluntario];
    console.log(params);
    const query = connect.con.query('DELETE FROM conf_voluntario where idConference = ? and idVoluntario = ?', params, function(err, rows, fields) {
        console.log(query.sql);
        if (!err) {
            res.status(jsonMessages.db.successDelete.status).send(jsonMessages.db.successDelete);
        }
...
module.exports = {
    ...
    readMembroComissao: readConfMembroComissao,
    saveMembroComissao: saveConfMembroComissao,
    deleteMembroComissao: deleteConfMembroComissao,
    readVoluntario: readConfVoluntario,
    saveVoluntario: saveConfVoluntario,
    deleteVoluntario: deleteConfVoluntario,
